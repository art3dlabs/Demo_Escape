...
import { addInteractableObject, removeInteractableObject } from './interaction.js'; // To manage pickup objects
...
export function addItemToInventory(itemName) {
    if (!itemName) { console.warn("addItemToInventory called with null/empty itemName"); return false; }
...
export function selectItem(itemName) {
    if (inventory.includes(itemName)) {
        ...
        document.body.classList.toggle('item-selected', getGameState() === GAME_STATES.PLAYING);
        ...
        if (getGameState() === GAME_STATES.PLAYING) {
             try {
                  const { getCamera, getScene, checkHoverInteraction } = await import('./interaction.js'); // Use dynamic import? Or pass camera/scene
                  checkHoverInteraction(getCamera(), getScene());
             } catch(e) { console.error("Error updating hover on item select:", e); }
        }
...
export function deselectItem(playSoundEffect = true) {
    if (selectedItem) {
        ...
        if (getGameState() === GAME_STATES.PLAYING) {
             try {
                 const { getCamera, getScene, checkHoverInteraction } = await import('./interaction.js');
                 checkHoverInteraction(getCamera(), getScene());
             } catch(e) { console.error("Error updating hover on item deselect:", e); }
        }
    }
}
...
export function handleInventoryClick(event) {
    if (getGameState() === GAME_STATES.INVENTORY && event.target.tagName === 'LI' && event.target.dataset.itemName) {
...
export function createPickupItem(scene, itemName, itemDescription, position, color = 0xffff00) {
    ...
    console.log(`${itemName} pickup created at ${position.x.toFixed(1)}, ${position.y.toFixed(1)}, ${position.z.toFixed(1)}`);
    return pickupMesh;
}
