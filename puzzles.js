...
import { getScene, getCamera, setGameState, GAME_STATES, areHintsEnabled } from './main.js'; // Import game state and utils
import { updateTooltip, showPuzzleModalContent, hidePuzzleModal, showMinigameUI, hideMinigameUI } from './ui.js';
import { addItemToInventory, removeItemFromInventory, getInventory, getSelectedItem, deselectItem, createPickupItem as createInvPickupItem } from './inventory.js'; // Needs access to inventory
import { addInteractableObject, removeInteractableObject, addCollisionObject, removeCollisionObject, getHeldObject, clearHoveredObject } from './interaction.js'; // Needs to manage objects
...
const AVAILABLE_REWARDS = [
    ...
    'Item_Llave_Pequeña', 'Item_Destornillador',
    'Clue_Codigo_Vent (789)', 'Clue_Codigo_Safe (123)',
    'Clue_Riddle (Tengo ojos...)', 'Clue_Color_Sequence (Rojo, Azul, Verde)',
    'Clue_Book_Sequence (Verde, Rojo, Violeta, Azul)', 'Item_Linterna_UV',
    'Clue_Password_Panel (HIDDEN)', 'Item_Diapositiva', 'Item_Bateria',
...
const allPuzzles = [
    // --- Holdable Cube & Pressure Plate Chain ---
    {
        ...
        setup: (s) => {
            ...
            const m = new THREE.MeshStandardMaterial({ color: 0x0077cc, metalness: 0.6, roughness: 0.4 });
    ...
    },
    {
        ...
        difficultyRestriction: null,
        setup: (s) => { // Configures the *existing* mesh created in sceneSetup.js
            const plateMesh = s.getObjectByName('pressurePlate');
            if (!plateMesh) { console.error("Pressure Plate mesh not found during setup!"); return; }
    ...
    // --- Key Lock Chest ---
    {
        ...
        difficultyRestriction: null,
        setup: (s) => { // Creates the chest mesh dynamically
            const g = new THREE.BoxGeometry(0.9, 0.5, 0.6);
            const chestMat = new THREE.MeshStandardMaterial({ color: 0x654321, roughness: 0.75, metalness: 0.05 }); // Simple wood
        ...
        interact: (chest, selectedItemName) => {
            const data = chest.userData;
            if (data.solved) { updateTooltip("El cofre ya está abierto."); return; }
            ...
            if (selectedItemName === data.requires) {
                ...
                // Grant reward (creates pickup inside/nearby)
                 ...
                 grantPuzzleReward(data.puzzleId, chest.position.clone().add(new THREE.Vector3(0, 0.3, 0))); // Spawn above chest

            } else if (getInventory().includes(data.requires)) {
                // Has key but not selected
                updateTooltip(`Necesitas seleccionar la ${data.requires} del inventario [I].`);
     ...
     // --- Simple Button ---
     {
         ...
         setup: (s) => {
             ...
             const m = new THREE.MeshStandardMaterial({ color: 0xff0000, metalness: 0.3, roughness: 0.7 });
             const b = new THREE.Mesh(g, m);
             b.position.set(getScene().getObjectByName('wallRight').position.x - 0.05, 1.5, 2.5); // On right wall
         ...
         interact: (button) => {
             ...
             if (!data.solved) {
                 ...
                 // Grant reward (create pickup nearby)
                  ...
                  grantPuzzleReward(data.puzzleId, button.position.clone().add(new THREE.Vector3(0, -0.5, 0.5))); // Spawn below/front

                 // Reset visual state after a delay (optional)
                 setTimeout(() => {
                     if (button.parent) { // Check if still exists
     ...
     // --- Desk Drawer ---
     {
         ...
         setup: (s) => {
             ...
             if (!drawerMesh) { console.error("Desk Drawer mesh not found during setup!"); return; }
             drawerMesh.userData = {
         ...
         interact: (drawer, selectedItemName) => {
             const data = drawer.userData;
             if (data.solved) { updateTooltip("El cajón ya está abierto."); return; }
             ...
             if (selectedItemName === data.requires) {
                 ...
                 // Grant reward
                  const { grantPuzzleReward } = await import('./puzzles.js');
                  grantPuzzleReward(data.puzzleId, drawer.position.clone().add(new THREE.Vector3(0, 0, -0.1))); // Spawn inside
         ...
         checkSolution: null,
         cleanup: (s) => { // Ensure drawer is reset visually if game restarts
             const drawerMesh = s.getObjectByName('deskDrawer');
     ...
     // --- Air Vent ---
     {
         ...
         setup: (s) => {
             ...
             if (!ventMesh) { console.error("Air Vent mesh not found during setup!"); return; }
             ventMesh.userData = {
         ...
         interact: (vent, selectedItemName) => {
             const data = vent.userData;
             if (data.revealed) { // Clue already found
                 updateTooltip(`Ya encontraste la pista en la rejilla.`);
                 playSound('click_soft');
                 return;
             }

             if (!data.opened) { // --- Trying to Open ---
                 if (selectedItemName === data.requires_open) {
                     ...
                     updateTooltip(`Usaste el ${selectedItemName}. La rejilla está abierta pero dentro está oscuro.`);
                     removeItemFromInventory(selectedItemName);
                     deselectItem();
             ...
             } else { // --- Vent is Open, Trying to Reveal ---
                 if (selectedItemName === data.requires_reveal) {
                     ...
                     updateTooltip(`Alumbras dentro con la ${selectedItemName}... ¡Hay una pista escrita! (Añadida al inventario)`);
                     ...
                     updateTooltip(`Está oscuro ahí dentro. Necesitas seleccionar la ${data.requires_reveal}.`);
                     playSound('click_soft');
                 } else {
         ...
         checkSolution: null,
         cleanup: (s) => { // Ensure reset if game restarts
              const ventMesh = s.getObjectByName('airVent');
              if(ventMesh && ventMesh.userData.originalPos && ventMesh.userData.originalRot) {
      ...
      // --- Lift Rug ---
      {
          ...
          setup: (s) => {
              ...
              if (!rugMesh) { console.error("Rug mesh not found!"); return; }
              rugMesh.userData = {
          ...
          interact: (rug) => {
              ...
              if (data.solved) { updateTooltip("Ya miraste debajo de la alfombra."); return; }
              ...
              updateTooltip(`Levantas la alfombra y encuentras una pista escrita en el suelo. (Añadida al inventario)`);
          },
          checkSolution: null,
          cleanup: (s) => { // Ensure reset
                const rugMesh = s.getObjectByName('rug');
     ...
     // --- Move Picture ---
     {
         ...
         setup: (s) => {
             ...
             if (!picGroup) { console.error("Picture Group not found!"); return; }
             picGroup.userData = {
         ...
         interact: (picGroup) => {
             ...
             if (data.solved) { updateTooltip("Ya moviste este cuadro."); return; }
     ...
     // --- Symbol Matching Book ---
     {
         ...
         setup: (s) => {
             ...
             if (!bookMesh) { console.error("Symbol Matching object not found!"); return; }
             bookMesh.userData = {
         ...
         interact: (book) => {
             ...
             if (data.solved) { updateTooltip("Libro de símbolos ya descifrado."); return; }
         ...
         checkSolution: (inputValue) => { // Called by puzzle modal submit
            ...
            const data = bookMesh.userData;
             ...
             if (inputValue.toUpperCase().trim() === correct) {
                 ...
                 if(puzzleMessage) puzzleMessage.textContent = `¡Correcto! Has descifrado la palabra. (Pista añadida)`;
     ...
     // --- Combination Lock Safe ---
     {
         ...
         rewardType: 'Item', // Contains an item
         difficultyRestriction: null,
         setup: (s) => { // Creates the safe mesh dynamically
             const safeGeo = new THREE.BoxGeometry(0.6, 0.6, 0.6);
             const safeMat = new THREE.MeshStandardMaterial({ color: 0xd3d3d3, metalness: 0.7, roughness: 0.4 });
             const safeMesh = new THREE.Mesh(safeGeo, safeMat);
             safeMesh.name = "demo_comboLock_Safe";
             safeMesh.castShadow = true; safeMesh.receiveShadow = true;
             const shelfMesh = s.getObjectByName('safeShelfMesh'); // Place on shelf
             if (shelfMesh) {
                 safeMesh.position.set(shelfMesh.position.x, shelfMesh.position.y + 0.04 / 2 + 0.6 / 2 + 0.01, shelfMesh.position.z);
             } else { safeMesh.position.set(-3.0, 1.3, -5.5); } // Fallback position
         ...
         interact: (safe) => {
             ...
             if (data.solved) { updateTooltip("La caja fuerte ya está abierta."); return; }

             // Check requirement (Safe Code Clue)
             const reqCheck = checkRequirements(data.requires);
             if (!reqCheck.met) {
                 updateTooltip(`Necesitas la pista "${data.requires}" para saber el código.`);
                 playSound('error_short');
                 return;
             }
              // Extract code from clue in inventory
              const clueItem = getInventory().find(item => item.startsWith(data.requires));
              const match = clueItem ? clueItem.match(/\(([^)]+)\)/) : null;
              const codeFromClue = (match && match[1]) ? match[1] : null;

              if (!codeFromClue) {
                   updateTooltip(`Error: No se pudo extraer el código de la pista "${clueItem}".`);
                   playSound('error_short');
                   return;
              }
         ...
         checkSolution: (inputValue) => {
             ...
             if (inputValue.trim() === correct) {
                 ...
                 // Grant Reward (Item inside)
                  const { grantPuzzleReward } = await import('./puzzles.js');
                  grantPuzzleReward(data.puzzleId, safeMesh.position.clone().add(new THREE.Vector3(0,0,0.4))); // Spawn in front

                 data.hint = "Caja fuerte abierta";
                 // Update modal and close
                 const puzzleMessage = getUIElement('puzzleMessage');
                 if(puzzleMessage) puzzleMessage.textContent = '¡Código Correcto! La caja se abre.';
     ...
     // --- Picture Frame (Simple Interaction Clue) ---
     {
         ...
         difficultyRestriction: null,
         setup: (s) => { // Creates the frame dynamically
             ...
             if (targetShelf) {
                 f.position.set(targetShelf.position.x, targetShelf.position.y + 0.04/2 + 0.4/2 + 0.01, targetShelf.position.z + 0.1);
                 f.rotation.copy(targetShelf.rotation);
                 f.rotation.x += Math.PI / 12; // Lean against wall slightly
             } else {
                 f.position.set(-ROOM_SIZE.width/2+0.15, 1.8, 2); f.rotation.y=Math.PI/2; // Fallback position
             }
         ...
         interact: (frame) => {
             ...
             updateTooltip("Miras detrás del marco y encuentras un papel con algo escrito. (Pista añadida al inventario)");
         },
         checkSolution: null,
         cleanup: null // Dynamic, removed by main cleanup
     },
     ...
     // --- Riddle Panel ---
     {
         ...
         difficultyRestriction: null,
         setup: (s) => { // Creates the panel dynamically
         ...
         interact: (panel) => {
             ...
             if (data.solved) { updateTooltip("Acertijo ya resuelto."); return; }
             ...
             showPuzzleModalContent(
                 'demo_riddle', data.puzzleId, 'Acertijo',
         ...
         checkSolution: (inputValue) => {
             ...
             if (inputValue.toLowerCase().trim() === correct) {
                 ...
                 // Update modal and close
                 const puzzleMessage = getUIElement('puzzleMessage');
                 if(puzzleMessage) puzzleMessage.textContent = `¡Correcto! La respuesta es ${correct}. (Pista añadida)`;
                 setTimeout(hidePuzzleModal, 1800);
                 return true;
             } else {
                 playSound('error_short');
      ...
      // --- Color Sequence Buttons ---
      {
          ...
          difficultyRestriction: null,
          setup: (s) => {
              const colors = { red: 0xff0000, blue: 0x0000ff, green: 0x00ff00, yellow: 0xffff00 }; // Available button colors
              const correctSequence = ['red', 'blue', 'green']; // The solution
              const state = { // Shared state for all buttons of this puzzle
              ...
              Object.keys(colors).forEach((id, index) => {
                  ...
                  b.position.set(startX + index * spacing, 1.8, getRoomSize().depth / 2 - 0.1); // On front wall
          ...
          interact: (button) => {
              ...
              if (!state || state.solved) {
                   updateTooltip(state?.solved ? "Secuencia ya completada." : "Error de estado del puzzle.");
                   return;
              }
              ...
              const reqCheck = checkRequirements(data.requires);
              if (!reqCheck.met) {
                  updateTooltip(`Necesitas la pista "${data.requires}" para saber la secuencia.`);
                  playSound('error_short');
                  return;
              }
              ...
              console.log(`Button pressed: ${buttonId}. Attempt: [${state.currentAttempt.join(', ')}]`);
              playSound('button_press');
               ...
               setTimeout(() => {
                   if(button.parent){ // Check if exists
              ...
              const expectedButton = state.correctSequence[state.currentAttempt.length - 1];
              if (buttonId !== expectedButton) {
                  ...
                  updateTooltip(`Correcto (${state.currentAttempt.length}/${state.correctSequence.length}). Siguiente...`);
                  // Sound feedback for correct step (optional)
                  // playSound('click_soft');
              }
          },
          checkSolution: null, // Interaction handles solution
          cleanup: (s) => { // Remove buttons dynamically created
                const firstButton = s.children.find(o => o.name?.startsWith('demo_colorButton_'));
      ...
      // --- Book Swap Puzzle ---
      {
          ...
          setup: (s) => {
              ...
              console.log(`Book puzzle configured for ${booksFound} books. Waiting for enable signal.`);
          },
          interact: null, // Handled by handleBookClick (called from interaction.js)
          checkSolution: null, // Handled by checkBookSequence
          cleanup: (s) => { // Reset book appearance on cleanup
               s.traverse((obj) => {
      ...
      // --- UV Hidden Message ---
      {
          ...
          difficultyRestriction: null,
          setup: (s) => { // Creates the plane dynamically
              ...
              const tH = createTextTexture(`PASSWORD:\n${passwordValue}`, 256, 256, '#AAAAEE', '#800080', 'bold 40px Consolas'); // Hidden text style
              const m = new THREE.MeshStandardMaterial({ map: tI, transparent: true, opacity: 0.1, side: THREE.DoubleSide }); // Almost transparent
          ...
          interact: (surface, selectedItemName) => {
              ...
              const clueRewardName = assignedRewards[data.puzzleId] || `Clue_Password_Panel (${data.correctCode})`;
              ...
              if (selectedItemName === data.requires) {
                  playSound('uv_reveal');
                   ...
                   updateTooltip(`Alumbras con la ${selectedItemName}... ¡Aparece un mensaje!`);
                  ...
                  data.revealTimeoutId = setTimeout(() => {
                      if (surface.parent) { // Check if still exists
                           surface.material.map = data.texture_initial;
                           surface.material.opacity = 0.1;
                           surface.material.needsUpdate = true;
                           if (!clueRevealed) updateTooltip("El mensaje vuelve a desaparecer..."); // Only show fade message if it was newly revealed
                      }
          ...
          checkSolution: null,
          cleanup: (s) => { // Dispose textures, clear timeout
              const surface = s.getObjectByName("demo_uvMessage_Surface");
              if (surface) {
                  if (surface.userData.revealTimeoutId) clearTimeout(surface.userData.revealTimeoutId);
      ...
      // --- Password Panel ---
      {
          ...
          setup: (s) => { // Creates the panel dynamically
              ...
              const m = new THREE.MeshStandardMaterial({ color: 0x401111, emissive: '#110000', emissiveIntensity: 0.8 }); // Reddish locked state
              const p = new THREE.Mesh(g, m);
          ...
          interact: (panel) => {
              const data = panel.userData;
              if (data.solved) { updateTooltip("Panel desbloqueado."); return; }

              const reqCheck = checkRequirements(data.requires);
              if (!reqCheck.met) {
                  updateTooltip(`Necesitas la pista "${data.requires}" para saber la contraseña.`);
                  playSound('error_short');
                  return;
              }
              ...
              // Requirement met, open puzzle modal
              showPuzzleModalContent(
                  'demo_passwordPanel', data.puzzleId, 'Panel de Acceso',
          ...
          checkSolution: (inputValue) => {
              ...
              const clueItem = getInventory().find(item => item.startsWith(data.requires));
              if (!clueItem) {
                  console.error("Password panel check failed: Required clue not found in inventory!");
                  const puzzleMessage = getUIElement('puzzleMessage');
                  if(puzzleMessage) puzzleMessage.textContent = 'Error: Falta la pista de la contraseña.';
    ...
    // --- Wires Minigame Trigger ---
    {
        ...
        setup: (s) => {
            ...
            if (!triggerMesh) { console.error("Wires Trigger mesh not found!"); return; }
        ...
        interact: (trigger) => {
            ...
            if (data.solved) { updateTooltip("Ya arreglaste los cables."); return; }

            console.log(`Triggering minigame: ${data.minigameId}`);
            showMinigameUI(data.minigameId, () => {
    ...
    // --- Simon Says Minigame Trigger ---
    {
        ...
        setup: (s) => {
            ...
            if (!triggerMesh) { console.error("Simon Says Trigger mesh not found!"); return; }
            triggerMesh.userData = {
                ...triggerMesh.userData,
                puzzleId: 'simonSays',
                requires: null,
        ...
        interact: (trigger) => {
            const data = trigger.userData;
            if (data.solved) { updateTooltip("Ya completaste la secuencia Simon Says."); return; }
      ...
      // --- Projector Puzzle ---
      {
          ...
          setup: (s) => {
              ...
              if (!projMesh) { console.error("Projector mesh not found!"); return; }
              projMesh.userData = {
          ...
          interact: (projector, selectedItemName) => {
              const data = projector.userData;
              if (data.solved) { updateTooltip("Ya has usado el proyector."); return; }
              ...
              else if(selectedItemName && (selectedItemName === 'Item_Bateria' || selectedItemName === 'Item_Diapositiva')) {
              ...
              if (data.hasBattery && data.hasSlide) {
                  ...
                  data.hint = "Proyector mostrando imagen (Pista encontrada)";
                   updateTooltip(`El proyector se enciende y muestra una imagen con una pista. (Añadida al inventario)`);
      ...
      // --- Final Keypad ---
      {
          ...
          setup: (s) => {
              ...
              if (!keypadMesh) { console.error("Final Keypad mesh not found!"); return; }
              keypadMesh.userData = {
          ...
          interact: (keypad) => {
              ...
              if (data.solved) { updateTooltip("Teclado final ya utilizado."); return; }
              ...
              // Requirements met, open puzzle modal
               showPuzzleModalContent(
                   'finalKeypad', data.puzzleId, 'Teclado Final',
                   'Introduce el código combinado.',
                   'number', 'Código...' // Number input
               );
          },
          checkSolution: (inputValue) => {
              ...
              const clueSafeItem = getInventory().find(item => item.startsWith('Clue_Codigo_Safe'));
              const clueVentItem = getInventory().find(item => item.startsWith('Clue_Codigo_Vent'));

              if (!clueSafeItem || !clueVentItem) {
                   console.error("Final keypad check failed: Missing required clues in inventory!");
                   const puzzleMessage = getUIElement('puzzleMessage');
                   if(puzzleMessage) puzzleMessage.textContent = 'Error: Faltan pistas necesarias.';
                   return false; // Should not happen if interact check passed
              }
              ...
              if (!codeSafe || !codeVent) {
                    console.error("Final keypad check failed: Could not extract codes from clues.");
                    const puzzleMessage = getUIElement('puzzleMessage');
              ...
              if (inputValue.trim() === correctCombinedCode) {
                  ...
                  // Update modal and close
                  const puzzleMessage = getUIElement('puzzleMessage');
                  if(puzzleMessage) puzzleMessage.textContent = `¡Código Combinado Correcto!`;
                  setTimeout(hidePuzzleModal, 1800);
                  return true;
              } else {
                  playSound('error_short'); // Keypad reject sound
                  const puzzleMessage = getUIElement('puzzleMessage');
                  if(puzzleMessage) puzzleMessage.textContent = 'Código Combinado Incorrecto.';
                  const puzzleInput = getUIElement('puzzleInput');
                  if(puzzleInput) puzzleInput.value = '';
                  return false;
              }
    ...
    // --- Escape Door (Always Last) ---
    {
        ...
        description: 'La única salida. Parece necesitar una llave maestra o un código final.',
        ...
        setup: (s) => { // Configures the existing door mesh
            const doorMesh = s.getObjectByName('exitDoor');
            if (!doorMesh) { console.error("Exit Door mesh not found during setup!"); return; }
            ...
            doorMesh.userData.hint = doorMesh.userData.baseHint || "La puerta de salida...";
            doorMesh.userData.puzzleId = 'escapeDoor'; // Ensure puzzleId is set
             resetMeshVisualState(doorMesh); // Ensure closed color/state
            console.log(`Escape Door puzzle state reset. Requirement will be set by selectPuzzles.`);
        },
        interact: (door, selectedItemName) => {
            ...
            if (data.solved) { updateTooltip("La puerta ya está abierta."); return; }

            let canOpen = false;
            let usedItem = null;

            // Check if requirement is an item and if player selected it
            if (actualRequirement.startsWith('Item_') && selectedItemName === actualRequirement) {
                canOpen = true;
                usedItem = selectedItemName;
            }
            // Check if requirement is a clue and player has it (doesn't need selection)
            else if (actualRequirement.startsWith('Clue_') && getInventory().includes(actualRequirement)) {
            ...
            if (canOpen) {
                ...
                if (actualRequirement.startsWith('Item_')) {
                     if(getInventory().includes(actualRequirement)) {
                          updateTooltip(`Selecciona la ${actualRequirement} para usarla aquí.`);
                     } else {
                     ...
                     if(!getInventory().includes(actualRequirement)) {
                          ...
                          // This might happen if we required a modal for the final code clue.
                           updateTooltip(`Tienes la pista "${actualRequirement}", pero necesitas activarla en la puerta.`);
                     }
                } else {
                     updateTooltip(`La puerta está cerrada. Necesita: ${actualRequirement}`);
                }
                playSound('locked_drawer'); // Locked sound
            }
        },
...
export function selectPuzzles(difficultyValue, scene) {
    console.log(`--- Selecting Puzzles (Difficulty Value: ${difficultyValue}) ---`);
    activePuzzles = [];
    ...
    if (difficultyValue === 4) { // Easy
        targetPuzzleCount = 4;
        difficultyName = 'Easy';
        filteredPuzzlePool = filteredPuzzlePool.filter(p => !p.difficultyRestriction); // Remove restricted
    } else if (difficultyValue === 7) { // Medium
        targetPuzzleCount = 7;
        difficultyName = 'Medium';
        filteredPuzzlePool = filteredPuzzlePool.filter(p => !p.difficultyRestriction); // Remove restricted
    } else if (difficultyValue === 10) { // Difficult
        targetPuzzleCount = 10;
        difficultyName = 'Difficult';
        // No filter needed, all puzzles allowed
    } else { // Expert (All) or Invalid -> Default to Expert
    ...
    if (targetPuzzleCount > filteredPuzzlePool.length) {
        console.warn(`Requested ${targetPuzzleCount} puzzles for ${difficultyName}, but only ${filteredPuzzlePool.length} suitable puzzles available. Using ${filteredPuzzlePool.length}.`);
        targetPuzzleCount = filteredPuzzlePool.length;
    }

    console.log(`Targeting ${targetPuzzleCount} puzzles for ${difficultyName} difficulty.`);
    ...
    while (puzzleChainIds.length < targetPuzzleCount && remainingPool.length > 0 && safetyCounter < maxLoops) {
        safetyCounter++;

        // Find candidates whose requirements are met by simulated inventory
        let candidates = remainingPool.filter(p => {
            const reqCheck = checkRequirementsSimulated(p.requires, currentSimulatedInventory);
            return reqCheck.met;
        });
        ...
        if (candidates.length > 0) {
            ...
            if (nextPuzzle.rewardType === 'Item' || nextPuzzle.rewardType === 'Clue') {
                 if (availableRewardsPool.length > 0) {
                     ...
                     let rewardIndex = Math.floor(Math.random() * availableRewardsPool.length);
                     ...
                     console.log(`   Assigned reward: ${rewardToAssign} to ${nextPuzzle.id}`);
                     // Add reward to simulated inventory for next checks
                      // Use prefix for items/clues, full name for signals
                      const rewardIdentifier = rewardToAssign.startsWith('Enable_') ? rewardToAssign : rewardToAssign.split(' ')[0];
                      currentSimulatedInventory.push(rewardIdentifier);

                 } else {
                      console.warn(`   No available rewards left to assign to ${nextPuzzle.id}!`);
                 ...
                 if (nextPuzzle.id === 'demo_colorSequence') { // Assuming color sequence gives book enable signal
            ...
            console.error(`Chain generation stuck at ${puzzleChainIds.length} puzzles. No candidates found with inventory: [${currentSimulatedInventory.join(', ')}]. Remaining pool: [${remainingPool.map(p=>p.id).join(', ')}]`);
            break; // Stop generation
        }
    } // End while loop

    if (safetyCounter >= maxLoops) {
        console.error("Puzzle chain generation exceeded max loops. Chain may be incomplete or broken.");
    }
    if (puzzleChainIds.length < targetPuzzleCount) {
         console.warn(`Generated chain is shorter (${puzzleChainIds.length}) than targeted (${targetPuzzleCount}).`);
    ...
    if (activePuzzles.length > 0) {
        ...
        if (lastPuzzleReward && (lastPuzzleReward === 'Item_Llave_Maestra' || lastPuzzleReward.startsWith('Clue_Codigo_Final'))) {
            finalRequirement = lastPuzzleReward;
        } else {
             console.warn(`Last puzzle (${lastPuzzleInChain.id}) reward "${lastPuzzleReward}" is not Master Key or Final Code. Defaulting door requirement to ${finalRequirement}.`);
         ...
         console.warn("No puzzles generated in chain! Door requirement will be default.");
    }
    ...
    if (doorMesh && doorMesh.userData && doorPuzzleDef) {
        ...
        activePuzzles.push({ ...doorPuzzleDef, requires: finalRequirement, isSolved: false, isMarkedSolved: false }); // Add door to active list
        console.log(`Escape Door configured. Requires: ${finalRequirement}`);
    } else {
        console.error("Exit door mesh or definition not found during final configuration!");
    }

    console.log(`--- Puzzle Selection Complete ---`);
    console.log(`Active Puzzles (${activePuzzles.length}): [${activePuzzles.map(p => `${p.id}${p.assignedReward ? '->'+p.assignedReward : ''}`).join(', ')}]`);
    console.log(`Assigned Rewards Map:`, assignedRewards);
    console.log(`Effective puzzle count for this game (excluding door): ${getActivePuzzlesCount()}`);
}
...
export function setupPuzzles(scene) {
    ...
    // Ensure puzzle-specific states are reset before setup
     bookPuzzleSolved = false; firstBookClicked = null; bookPuzzleEnabled = false;
...
export function cleanupPuzzles(scene) {
    ...
    const staticPuzzleMeshNames = [ // Names of meshes created in sceneSetup that are configured by puzzles
         "deskDrawer", "airVent", "pressurePlate", "finalKeypad",
         "wiresPuzzle_Trigger", "simonSays_Trigger", "symbolMatching_Object",
         "projectorPuzzle_Object", "pictureOnWall", "rug",
         "exitDoor",
    ...
    scene.traverse((child) => {
        if ( child.userData && (child.userData.puzzleId || child.userData.isPickup || child.userData.holdable) ) {
    ...
    console.log(`Removing ${objectsToRemove.length} dynamic puzzle/pickup objects.`);
    objectsToRemove.forEach(o => {
        removeInteractableObject(o);
        removeCollisionObject(o);
        if (o === getHeldObject()) { // Make sure interaction.js exports this getter or manages it
    ...
    staticPuzzleMeshNames.forEach(name => {
        ...
        if (mesh && mesh.userData) {
             ...
             if(mesh.userData.hasBattery !== undefined) mesh.userData.hasBattery = false;
    ...
    bookPuzzleEnabled = false; bookPuzzleSolved = false; firstBookClicked = null;
...
export function grantPuzzleReward(solvedPuzzleId, position) {
    ...
    if (!rewardName) {
        console.warn(`Cannot grant reward: No assigned reward found for puzzle ${solvedPuzzleId}`);
        return;
    }
    ...
    if (rewardName.startsWith('Item_')) {
        ...
        const spawnPos = position ? position.clone().add(new THREE.Vector3(0, 0.2, 0)) : new THREE.Vector3(0, 0.5, 0); // Adjust offset
        ...
        createInvPickupItem(getScene(), rewardName, `Un ${rewardName}`, spawnPos, itemColor); // Use inventory's function
        ...
        if (addItemToInventory(rewardName)) {
         ...
         console.log(`Signal reward type used for ${solvedPuzzleId}: ${rewardName}. Signal should be handled by receiving logic.`);
        ...
        console.warn(`Unknown reward type assigned to ${solvedPuzzleId}: ${rewardName}`);
    }
}
...
export function markPuzzleSolved(puzzleId) {
    ...
    if (puzzleIndex > -1 && !activePuzzles[puzzleIndex].isMarkedSolved) {
        ...
        console.log(`Puzzle "${activePuzzles[puzzleIndex].name || puzzleId}" marked solved. Total Solved: ${puzzlesSolvedCount}/${getActivePuzzlesCount()}`);
        ...
        if (puzzleId === 'escapeDoor') {
             ...
             console.log("Escape Door marked solved, victory sequence should trigger.");
        }

    } else if (puzzleIndex > -1 && activePuzzles[puzzleIndex].isMarkedSolved) {
        console.log(`Puzzle ${puzzleId} was already marked as solved.`);
    } else {
        console.warn(`Attempted to mark puzzle solved which is not active or not found: ${puzzleId}`);
    }
}
...
export function checkRequirements(requirements) {
    if (!requirements) return { met: true, missing: [], needsItem: false, itemNeeded: null };
    ...
    reqArray.forEach(req => {
        ...
        if (req === 'Enable_Book_Puzzle') {
            if (bookPuzzleEnabled) { // Check the global flag
                requirementMet = true;
            } else {
...
function checkRequirementsSimulated(requirements, simulatedInventory) {
    ...
    reqArray.forEach(req => {
        let requirementMet = false;
        if (req === 'Enable_Book_Puzzle') {
            if (simulatedInventory.includes('Enable_Book_Puzzle')) requirementMet = true;
        } else if (req.startsWith('Item_') || req.startsWith('Clue_')) {
            const prefix = req.split(' ')[0];
            // Check if the *prefix* exists in the simulated inventory
            if (simulatedInventory.some(itemPrefix => itemPrefix === prefix)) requirementMet = true;
        }
...
export function handleBookClick(clickedBook) {
    if (!bookPuzzleEnabled || bookPuzzleSolved || !clickedBook || !clickedBook.userData?.interactable) {
        if (!bookPuzzleEnabled && !bookPuzzleSolved) { updateTooltip("Estos libros no parecen moverse..."); }
        else if (bookPuzzleSolved) { updateTooltip("La secuencia de libros ya es correcta."); }
        return;
    }
     ...
     if (!reqCheck.met) {
          updateTooltip(`Necesitas la pista ${reqCheck.missing.join(', ')} para saber el orden.`);
          playSound('error_short');
          return;
     }
    ...
    if (bookIndex === undefined) { console.warn("Clicked book is missing bookIndex!", clickedBook); return; }
    ...
    if (firstBookClicked === null) {
        ...
        updateTooltip(`Seleccionado: Libro ${bookIndex + 1}. Haz clic en otro para intercambiar.`);
    } else {
        // Second book clicked
        ...
        if (firstBookClicked === clickedBook) {
            ...
            console.log(`Swapping colors between book ${firstBookClicked.userData.bookIndex + 1} and ${bookIndex + 1}`);
...
function checkBookSequence() {
    if (bookPuzzleSolved) return; // Already solved

    const bookMeshes = [];
    getScene().traverse((obj) => { // Find all book meshes in the current scene
        if (obj.name?.startsWith('bookPuzzle_')) { bookMeshes.push(obj); }
    });
    // Sort by original index to ensure consistent order checking
    bookMeshes.sort((a, b) => (a.userData?.bookIndex || 0) - (b.userData?.bookIndex || 0));

    if (bookMeshes.length !== bookSwapCorrectSequence.length) {
        console.error("Book sequence check failed: Mismatch between found books and correct sequence length!");
        return;
    }

    let match = true;
    for (let i = 0; i < bookMeshes.length; i++) {
        if (!bookMeshes[i].material || bookMeshes[i].material.color.getHex() !== bookSwapCorrectSequence[i]) {
            match = false;
            break;
        }
    }
...
function createTextTexture(text, width = 256, height = 128, bgColor = '#333', textColor = '#FFF', font = '20px Arial') {
