...
import {
  ...
  getPuzzleDefinition,
} from "./puzzles.js"; // To get puzzle info for modals
...
export function updateHUD() {
  ...
  if (
    !uiElements.puzzlesSolvedText ||
    !uiElements.objectiveText ||
    !uiElements.timerText
  )
    return;
  ...
  if (uiElements.puzzlesTotalText)
    uiElements.puzzlesTotalText.textContent = totalCount; // Update total in HUD
  if (uiElements.pausePuzzlesCount)
    uiElements.pausePuzzlesCount.textContent = solvedCount; // Update pause menu count
  ...
  if (
    ...
    gameState === GAME_STATES.GAMEOVER_TIMEUP
  ) {
    if (uiElements.timerDisplay)
      uiElements.timerDisplay.style.display = "block";
...
export function getDifficultyValue() {
  return uiElements.difficultySelect
    ? parseInt(uiElements.difficultySelect.value, 10)
    : 4; // Default Easy
}

export function updateMenuPuzzleCountDisplay() {
  if (!uiElements.menuPuzzleCountDisplay || !uiElements.difficultySelect)
    return;
...
export function showPuzzleModalContent(
  ...
  if (uiElements.puzzleDescription)
    uiElements.puzzleDescription.textContent = description;
...
function submitPuzzleModalInput() {
  if (
    ...
    if (puzzleDef && typeof puzzleDef.checkSolution === "function") {
      ...
      if (uiElements.puzzleMessage)
        uiElements.puzzleMessage.textContent =
          "Error: No se puede verificar la solución.";
    }
  } else {
    console.warn(
...
const minigameImplementations = {
  wires: {
    init: (areaElement, instructionsElement, winCallback, loseCallback) => {
      ...
      areaElement
        .querySelectorAll(".minigame-sim-btn")
        .forEach((btn, index) => {
          btn.onclick = () => {
            if (index === 0) winCallback();
            else loseCallback();
          };
        });
    },
    cleanup: (areaElement) => {
      areaElement.innerHTML = ""; /* Remove specific listeners */
    },
  },
  simon: {
    init: (areaElement, instructionsElement, winCallback, loseCallback) => {
...
export function showMinigameUI(minigameId, successCallback) {
  if (
    !uiElements.minigameArea ||
    !uiElements.minigameTitle ||
    !uiElements.minigameInstructions
  ) {
  ...
  uiElements.minigameTitle.textContent =
    minigameId === "wires"
      ? "Conectar Cables"
      : minigameId === "simon"
      ? "Simon Says"
...
export function setupUIEventListeners() {
  ...
  if (uiElements.blocker && uiElements.startPrompt) {
    uiElements.blocker.addEventListener("click", (e) => {
      if (
        getGameState() === GAME_STATES.MENU &&
        e.target === uiElements.startPrompt
      ) {
  ...
  document.addEventListener("keydown", (e) => {
    ...
    if (e.key === "Escape") {
      ...
      switch (gameState) {
        ...
        case GAME_STATES.PUZZLE_2D:
          ...
          console.warn(
            "Escape in PUZZLE_2D - No specific handler, should use button or lock."
          );
          break;
        case GAME_STATES.INVENTORY:
          setIgnoreNextUnlockFlag(true);
          hideInventoryModal();
          break; // Call specific hide function
        case GAME_STATES.MINIGAME:
          setIgnoreNextUnlockFlag(true);
          const minigameId =
            uiElements.minigameTitle.textContent === "Conectar Cables"
              ? "wires"
              : uiElements.minigameTitle.textContent === "Simon Says"
              ? "simon"
              : null; // Infer ID hackily
          if (minigameId) hideMinigameUI(minigameId, true);
          else
            console.warn("Could not determine minigame ID to close on Escape.");
          break;
        case GAME_STATES.HELP_CONFIRM:
          setIgnoreNextUnlockFlag(true);
          hideHelpConfirmModal();
          break; // Hide help modal
    ...
    if (
      ...
      e.key === "Enter" &&
      document.activeElement === uiElements.puzzleInput
    ) {
  ...
  window.addEventListener("mousedown", (event) => {
    // Use mousedown for interaction responsiveness
    const gameState = getGameState();
  ...
  addListener("minigameCloseButton", "click", () => {
    ...
    const minigameId =
      uiElements.minigameTitle.textContent === "Conectar Cables"
        ...
        : null;
    if (minigameId) hideMinigameUI(minigameId, true);
    else console.warn("Could not determine minigame ID to close.");
  });
  ...
  addListener("confirmHelpBtn", "click", () => {
    ...
    if (uiElements.helpButton) {
      uiElements.helpButton.disabled = true;
      uiElements.helpButton.style.opacity = 0.6;
      uiElements.helpButton.style.cursor = "not-allowed";
      uiElements.helpButton.textContent = "Pistas Activadas";
    }
