...
// --- Interaction Check ---
export function checkHoverInteraction(camera, scene) {
  // Pass camera and scene
  ...
  if (heldObject) {
    const holdHint = `Llevando: ${
      heldObject.userData?.name || "objeto"
    }. [E] / Clic Izq para soltar.`;
  ...
  for (const hit of hits) {
    ...
    if (obj && obj.userData && hit.distance < hitDistance) {
      // Check if it's a registered interactable
  ...
  if (currentHoverTarget) {
    ...
    const isActivePuzzle =
      puzzleId && activePuzzles.some((p) => p.id === puzzleId);
    const puzzleDef = isActivePuzzle
      ? activePuzzles.find((p) => p.id === puzzleId)
      : null;
    ...
    if (isActivePuzzle && puzzleDef) {
      ...
      if (isSolved && puzzleId !== "airVent" && puzzleId !== "demo_uvMessage") {
        // Allow re-interaction for some
        hint = hintsOn
          ? `PISTA: ${baseHint} (Ya resuelto)`
          : `${baseHint} (Resuelto)`;
        ...
        if (!reqCheckResult.met) {
          hint = hintsOn
            ? `PISTA: Falta ${reqCheckResult.missing.join(", ")}`
            : `Necesitas ${reqCheckResult.missing.join(", ")}`;
          canInteractNow = true; // Can still interact to get the message
        } else {
          const needsItemSelected =
            reqCheckResult.needsItem &&
            !data.holdable &&
            !data.isPickup &&
            !data.isMinigameTrigger;
          const requiredItemPrefix = reqCheckResult.itemNeeded
            ? reqCheckResult.itemNeeded.split(" ")[0]
            : null;
          const selectedItemPrefix = selectedItem
            ? selectedItem.split(" ")[0]
            : null;

          if (needsItemSelected && !selectedItem) {
            hint = hintsOn
              ? `PISTA: Selecciona ${reqCheckResult.itemNeeded} del inventario [I]`
              : `Selecciona ${reqCheckResult.itemNeeded}`;
            canInteractNow = true; // Interact shows this message
          } else if (
            needsItemSelected &&
            selectedItem &&
            requiredItemPrefix !== selectedItemPrefix
          ) {
            hint = hintsOn
              ? `PISTA: No puedes usar ${selectedItem}. Necesitas ${reqCheckResult.itemNeeded}`
              : `Objeto incorrecto: ${selectedItem}`;
            ...
            hint = hintsOn
              ? `PISTA: Activa el minijuego "${puzzleDef.name}" [E]`
              : `Activar ${puzzleDef.name} [E]`;
            canInteractNow = true;
          } else {
            // Requirements met, no specific item needed or correct item selected
            hint = hintsOn
              ? `PISTA: Interactúa con ${puzzleDef.name} [E]`
              : data.hint || puzzleDef.description || `Interactuar [E]`;
            canInteractNow = true;
          }
        }
        ...
        if (
          puzzleId === "bookSwap" &&
          puzzleDef.requires?.includes("Enable_Book_Puzzle") &&
          !checkRequirements("Enable_Book_Puzzle").met &&
          !isSolved
        ) {
          hint = hintsOn
            ? "PISTA: Estos libros no se moverán hasta que actives algo más."
            : "Estos libros no se mueven...";
    ...
    // 2. Holdable Objects
      ...
      hint = hintsOn
        ? `PISTA: Puedes recoger ${data.name || "este objeto"} [E]`
        : `Recoger ${data.name || "objeto"} [E]`;
      canInteractNow = true;
    }
    // 3. Pickup Items
    else if (data.isPickup && data.itemName) {
      hint = hintsOn
        ? `PISTA: Recoge ${data.itemName} [E]`
        : `Recoger ${data.itemName} [E]`;
      canInteractNow = true;
    }
    // 4. Static Furniture / Non-Puzzle Interactables
    else if (data.interactable && !puzzleId) {
      // It's interactable but not part of the current puzzle chain
      hint = hintsOn
        ? `PISTA: Puedes examinar ${data.hint || "esto"}. No parece útil ahora.`
        : data.hint || "Examinar";
      ...
      if (hintsOn) {
        if (data.canBeMoved)
          hint = `PISTA: Este cuadro (${currentHoverTarget.name}) parece que puede moverse...`;
        if (data.canBeLifted)
          hint = `PISTA: Quizás haya algo debajo de esta alfombra (${currentHoverTarget.name})...`;
    ...
    // 5. Puzzle Objects NOT Active in current game (e.g., keypad on easy mode)
      ...
      hint = hintsOn
        ? `PISTA: ${
            data.baseHint || "Esto"
          } no es necesario para escapar en esta dificultad.`
        : data.baseHint || "No parece útil ahora.";
      canInteractNow = false;
    }
    // 6. Non-Interactable
    else {
...
export function handleInteraction() {
  ...
  if (hoveredObject && hoveredObject.userData) {
    ...
    const isActivePuzzle =
      puzzleId && getActivePuzzles().some((p) => p.id === puzzleId);
    const selectedItem = getSelectedItem(); // Get currently selected inventory item

    console.log(
      `Attempting interaction with: ${
        obj.name || "Unnamed"
      } (Active: ${isActivePuzzle}, PuzzleID: ${puzzleId || "None"})`
    );
    ...
    if (puzzleDef) {
      ...
      if (
        needsItemSelected &&
        selectedItem &&
        !reqCheck.itemNeeded.startsWith(selectedItem.split(" ")[0])
      ) {
        updateTooltip(
          `No puedes usar ${selectedItem} aquí. Necesitas ${reqCheck.itemNeeded}.`
        );
        playSound("error_short");
      ...
      if (typeof puzzleDef.interact === "function") {
        ...
        if (isActivePuzzle) {
          ...
          playSound("click_soft");
          console.log(
            `Interaction stopped: ${obj.name} (${puzzleId}) is not an active puzzle.`
          );
        }
      } else {
...
function pickupItem(itemMesh) {
  if (!itemMesh?.userData?.isPickup || !itemMesh.userData.itemName) {
    console.warn("pickupItem failed: Invalid mesh or data.");
    return;
  }
  ...
  if (addItemToInventory(itemName)) {
    // addItemToInventory handles checks & UI update
...
function pickupHoldableObject(obj) {
  ...
  setTimeout(() => {
    if (heldObject === obj && getGameState() === GAME_STATES.PLAYING) {
      // Check if still holding and playing
      const holdHint = `Llevando: ${
        heldObject.userData.name || "objeto"
      }. [E] / Clic Izq para soltar.`;
...
export async function placeHeldObject(forceDrop = false) {
  // <<< Make 100% sure 'async' is present here  // <<< MUST HAVE 'async' HERE
  ...
  if (!forceDrop && gameState === GAME_STATES.PLAYING) {
    ...
    const hits = interactionRaycaster.intersectObjects(
      getCollisionObjects(),
      true
    ); // Intersect with COLLISION objects
    let surfaceHit = null;
    ...
    for (const hit of hits) {
      ...
      if (hit.object !== placingObject && hit.distance <= DIST_PLACE) {
        ...
        if (norm.y > 0.7 && !isPlayerHit) {
          // Angle threshold for 'flat enough'
          surfaceHit = hit;
    ...
    if (surfaceHit) {
      ...
      console.log(
        `Found surface to place on: ${targetSurface.name || "Unnamed Surface"}`
      );
      ...
      const finalPos = placementPoint
        .clone()
        .add(surfaceHit.face.normal.multiplyScalar(objectHeight / 2 + 0.01)); // Add tiny offset too
      ...
      console.log(
        `Placed ${placingObjectName} at ${finalPos.x.toFixed(
          2
        )}, ${finalPos.y.toFixed(2)}, ${finalPos.z.toFixed(2)}`
      );
      placedSuccessfully = true;

      // *** PRESSURE PLATE LOGIC ***
      if (
        targetSurface.name === "pressurePlate" &&
        placingObjectData.puzzleId === "demo_holdableCube"
      ) {
        ...
        if (
          plateObject &&
          plateObject.userData &&
          !plateObject.userData.solved &&
          platePuzzleDef
        ) {
          ...
          if (platePuzzleDef.assignedReward) {
            // Check if a reward was assigned
  ...
  if (!placedSuccessfully) {
    console.log(
      forceDrop
        ? "Forcing drop."
        : "No suitable surface found, dropping object."
    );
    ...
    cameraDir.y = 0;
    cameraDir.normalize(); // Horizontal direction player is facing

    const dropPosBase = playerPos
      .clone()
      .add(cameraDir.multiplyScalar(dropOffsetForward));
    ...
    scene.add(placingObject);
    placingObject.updateMatrixWorld();
    const bb = new THREE.Box3().setFromObject(placingObject);
    ...
    if (groundHits.length > 0) {
      ...
      const firstValidHit = groundHits.find(
        (hit) => hit.object !== placingObject
      ); // Don't hit self
      if (firstValidHit) {
    ...
    const finalDropPos = new THREE.Vector3(
      dropPosBase.x,
      groundY + objectHeight / 2 + 0.01,
      dropPosBase.z
    );
    ...
    placedSuccessfully = true;
    console.log(
      `Dropped ${placingObjectName} at ${finalDropPos.x.toFixed(
        2
      )}, ${finalDropPos.y.toFixed(2)}, ${finalDropPos.z.toFixed(2)}`
    );
  }
  ...
  if (placedSuccessfully) {
    ...
    if (placingObjectData.holdable) {
      // Only add back if it was originally holdable
    ...
    if (
      ...
      console.log(
        "Holdable Cube puzzle marked as solved upon being placed/dropped."
      );
    }
    ...
    console.error(
      `Failed to place or drop ${placingObjectName}! Object remains held.`
    );
...
export function dropSelectedItem() {
  ...
  if (removeItemFromInventory(itemToDrop)) {
    ...
    const controls = getControls();
    const camera = getCamera();
    const dropOffsetForward = 0.8;
    const dropOffsetUp = -0.4; // Relative to player eye level
    const playerPos = controls.getObject().position;
    ...
    const dropPosBase = playerPos
      .clone()
      .add(camDir.multiplyScalar(dropOffsetForward));
    dropPosBase.y += dropOffsetUp; // Adjust vertical position based on player height/view

    // Raycast downwards to find ground
    const downRay = new THREE.Raycaster(
      dropPosBase.clone().add(new THREE.Vector3(0, PLAYER_HEIGHT, 0)),
      new THREE.Vector3(0, -1, 0),
      0,
      PLAYER_HEIGHT * 1.5
    );
    const groundIntersects = downRay.intersectObjects(
      getCollisionObjects(),
      true
    );
    ...
    createInvPickupItem(
      ...
      `Dropped: ${itemToDrop}`,
      finalDropPos
    ); // Using inventory's create function
